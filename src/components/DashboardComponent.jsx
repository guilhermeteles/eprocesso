  ...
  return (
    <div className="grid h-full w-full p-8 gap-4 items-start overflow-auto sm:overflow-y-none lg:container sm:grid-cols-[repeat(auto-fit,minmax(380px,1fr))]">
      ...
      <div className='bg-white px-6 pb-6 pt-4 shadow-[0_3px_10px_rgb(0,0,0,0.1)] flex flex-col gap-2 h-fit'>
        ...
        <div className='sm:max-h-[calc(100vh-255px)] h-fit flex flex-col gap-4 overflow-auto w-full'>
              ...
              >
                <div className="flex justify-between items-center mb-2">
                  <span className="text-xs font-semibold flex items-center gap-2 w-100">
                    {/* Badge */}
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-semibold ${note.group === 'empty' ? 'hidden' : badgeColors[note.group]
                        }`}
                    >
                    ...
                    <span
                      className="flex-1 truncate min-w-0 w-32"
                      title={note.name}
                    >
                      {note.name}
                    </span>
